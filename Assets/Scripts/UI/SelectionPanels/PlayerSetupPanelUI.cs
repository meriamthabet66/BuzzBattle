using Data.Data;
using Data.DTO; // Ensure this matches your MatchSetupData location
using Managers;
using UnityEngine;
...
namespace UI {
    ...
    public class PlayerSetupPanelUI : MonoBehaviour {
        [Header("Where to go next?")] [SerializeField]
        private GameObject nextPanel; // Drag MatchConfigPanel here

        [Header("The 4 Input Sections")]
        [Tooltip("Drag your 4 PlayerInputSectionUI GameObjects here IN ORDER (Player 1, 2, 3, 4)")]
        [SerializeField]
        private PlayerInputSectionUI[] inputSections;
        ...
        public void OnClickNext() {
            ...
            for (int i = 0; i < MatchSetupData.PlayerCount; i++) {
                ...
                // Inside PlayerSetupPanelUI.cs -> OnClickNext()

                // for (int i = 0; i < MatchSetupData.PlayerCount; i++) {
                //     string playerName = inputSections[i].GetPlayerName();
                //     int charId = inputSections[i].GetCharacterId();
                //
                //     // --- NEW: Grab the linked cloud profile ---
                //     ProfileDTO linkedAccount = inputSections[i].GetLinkedProfile();
                //
                //     // Pass everything to the PlayerManager
                //     PlayerManager.Instance.AddPlayer(playerName, charId, linkedAccount);
                // }
